...
build() {
	docker compose -f $yml build --build-arg REGISTRY='hclcr.io/wa/workload-automation/hcl-' --build-arg VERSION=10.2.6
    docker compose -f $yml up --no-start
}

build_start() {
	docker compose -f $yml build --build-arg REGISTRY='hclcr.io/wa/workload-automation/hcl-' --build-arg VERSION=10.2.6 $1
...
start_config() {
    if ! docker image inspect "hclcr.io/wa/workload-automation/hcl-aida-config:10.2.6" >&/dev/null; then
