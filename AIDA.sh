...
build() {
	$CONTAINER_RUNTIME compose -f $yml build --build-arg REGISTRY='wa-registry.prod.hclpnp.com/wa-aida/aida-hcl/' --build-arg VERSION=10.2.6
    $CONTAINER_RUNTIME compose -f $yml up --no-start
}

build_start() {
	$CONTAINER_RUNTIME compose -f $yml build --build-arg REGISTRY='wa-registry.prod.hclpnp.com/wa-aida/aida-hcl/' --build-arg VERSION=10.2.6 $1
...
start_config() {
    if ! $CONTAINER_RUNTIME image inspect "wa-registry.prod.hclpnp.com/wa-aida/aida-hcl/aida-config:10.2.6" >&/dev/null; then
